...
class AnastellosEngine:
    def __init__(self, *, additional_guild_params={}, additional_global_params={}):
        ...
        if self.config.mode == 2:
            ...
            logging.getLogger('guilded').setLevel(20)
        else:
            logging.getLogger('guilded').propagate = False
        ...
        #     status = guilded.Status.dnd
        # elif self.config.mode == 1:
        #     activity = guilded.Game(
        #         f'{self.config.full_version} | Some bugs may occur.')
        #     status = guilded.Status.idle
        # else:
        #     activity = guilded.Game(f'{self.config.full_version}')
        #     status = guilded.Status.online

        # intents = guilded.Intents.all()
        # allowed_mentions = guilded.AllowedMentions.all()
        ...
        async def on_ready():
            ...
            # guildlist = ', '.join(
            #     ['"' + j.name + '"' for j in self.bot.servers])
            # _log.info(
